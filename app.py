...
async def main():
    ...
    try:
            ...
            while True:
                try:
                    ...
                    if best_track:
                        ...
                        console.print(f"[{C_DIM}][Headless] No active or upcoming tracks found. Polling again in 30s...[/{C_DIM}]")
                except Exception as poll_err:
                    console.print(f"[orange3][Headless] Polling error: {poll_err}. Reconnecting client and retrying in 30s...[/orange3]")
                    try:
                        await client.shutdown()
                    except Exception:
                        pass
        ...
        await client.shutdown()
        console.print(Panel(
            f"[bold {C_GOLD}]Agent shut down successfully. See you next race![/bold {C_GOLD}]",
            border_style=C_DIM,
            expand=False,
        ))
